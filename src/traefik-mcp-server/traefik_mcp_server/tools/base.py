    ...
    def __init__(self, service_locator: Dict[str, Any]):
        ...
        Args:
            service_locator: Dictionary of services providing:
                ...
                - migration_service: Service for NGINX to Traefik migration operations
        ...
        self.nginx_migration_service = service_locator.get('nginx_migration_service')
        
        config = service_locator.get('config')
        if config is None:
            raise ValueError("ServerConfig ('config') is strictly required in service_locator")
