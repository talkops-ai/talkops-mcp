...
def enforce_structured_size_limit(
    ...
    *,
    ...
    Args:
        ...
        query_hint: Optional TraceQL query string for generating refinement advice.
...
def _build_truncation_advice(
    ...
    parts = [
        ...
        "To get more precise results, refine your TraceQL query:",
    ]

    suggestions = [
        "- Add resource filters: { resource.service.name = \"x\" && resource.namespace = \"y\" }",
        "- Use status filters: { status = error }",
        "- Narrow the time range with 'since' parameter",
        "- Use max_spans parameter to limit spans per trace",
    ]

    if query_hint and "duration" in query_hint.lower():
        suggestions.insert(
            0,
            "- Use quantile_over_time() or histogram_over_time() "
            "instead of querying raw span data for duration analysis",
        )
