...
def enforce_structured_size_limit(
    ...
    *,
    ...
    Args:
        ...
        query_hint: Optional PromQL query string for generating refinement advice.
...
def _build_truncation_advice(
    ...
    parts = [
        ...
        "To get more precise results, refine your PromQL query:",
    ]

    suggestions = [
        "- Add label filters: metric_name{service=\"x\", namespace=\"y\"}",
        "- Use topk(N, ...) to limit to top N series",
        "- Use aggregation: sum by (label)(metric) or avg by (label)(metric)",
    ]

    if query_hint and "bucket" in query_hint.lower():
        suggestions.insert(
            0,
            "- Use histogram_quantile(0.95, sum by (le)(rate(metric[5m]))) "
            "instead of querying raw buckets",
        )
