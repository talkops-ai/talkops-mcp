...
or ``list`` parameters as serialised strings (a known MCP ecosystem issue).
"""
...
class JsonCoercionMiddleware(Middleware):
    ...
    tried first (fast, no ambiguity), then YAML (for structured config).
    ...
    def _try_parse(value: str) -> Any:
        """Try JSON first (fast), then YAML (for structured config).
...
def setup_middleware(mcp: FastMCP, config: ServerConfig) -> None:
    ...
    #   "destinations": "[{\"server\": \"...\", \"namespace\": \"...\"}]"  (JSON string)
    # instead of:
    #   "destinations": [{"server": "...", "namespace": "..."}]             (native list)
    mcp.add_middleware(JsonCoercionMiddleware())

    # 3. Caching — Store frequently requested data to improve performance
    # ArgoCD data changes frequently; use short TTLs for cluster-state queries.
    mcp.add_middleware(ResponseCachingMiddleware(
        ...
        # Short TTL for ArgoCD state: sync/deploy operations invalidate cache
        call_tool_settings=CallToolSettings(ttl=30, enabled=True),
        # Cache resource reads for 10 seconds - real-time data
        read_resource_settings=ReadResourceSettings(ttl=10, enabled=True),
