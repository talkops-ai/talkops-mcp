...
class PDFDocumentLoader(BaseDocumentLoader):
    ...
    Handles standardized metadata.
    """
    def load(self) -> List[Any]:
        try:
            is_url = self.source_path.startswith("http://") or self.source_path.startswith("https://")
            if is_url:
                resp = requests.get(self.source_path)
            ...
            raise LoaderError(f"Failed to load PDF file {self.source_path}: {e}")

    def extract_metadata(self) -> Dict[str, Any]:
        return generate_base_metadata(self.source_path, self.__class__.__name__)

register_loader('pdf', PDFDocumentLoader) 