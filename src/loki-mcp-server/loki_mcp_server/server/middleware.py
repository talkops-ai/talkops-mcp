...
Uses FastMCP's built-in middleware stack following the 7-layer pattern:
Error Handling → Response Limiting → Rate Limiting →
Transform → Caching → Logging → Timing
"""
...
def setup_middleware(mcp: FastMCP, config: ServerConfig) -> None:
    ...
    Middleware order (processed in registration order):
    ...
    # 5. Caching — list operations are static so a long TTL is fine.
    # Tool invocation caching is DISABLED. Loki log queries return
    # real-time data that changes continuously. Caching them means
    # an AI agent that retries a query during an active incident will
    # silently receive stale data. Read-resource caching is kept
    # because those resources are config-level static data.
    ...
    # 6. Logging — structured logging with payload details
