...
class OrchestrationService:
    ...
    def __init__(self, config, argo_service=None):
    ...
    async def configure_cost_aware_deployment(
        ...
        Tracks costs using a configurable per-pod-hour rate and provides
        optimization recommendations to stay within budget.
        
        ⚠️ DISCLAIMER: Costs are ESTIMATED using a simple formula:
           cost = replicas × cost_per_pod_hour
        This does NOT integrate with cloud provider billing APIs.
        For actual costs, use AWS Cost Explorer, GCP Billing, or Azure Cost Management.
        The `cost_per_pod_hour` parameter can be tuned to approximate your real costs.
        ...
        Args:
            ...
            cost_per_pod_hour: Cost per pod per hour in USD (default: $0.05)
        ...
        try:
            ...
            try:
                ...
                replicas_raw = rollout_status.get("replicas", 3)
                if isinstance(replicas_raw, dict):
                    replicas = replicas_raw.get("total", 3) or rollout_status.get("desired_replicas", 3)
                else:
                    replicas = replicas_raw if isinstance(replicas_raw, int) else 3
            except Exception:
    ...
    async def validate_deployment_policy(
        ...
        try:
            ...
            try:
                ...
                # Check replica count — replicas may be dict or int
                replicas_raw = rollout_status.get("replicas", 0)
                if isinstance(replicas_raw, dict):
                    replicas = replicas_raw.get("total", 0) or rollout_status.get("desired_replicas", 0)
                else:
                    replicas = replicas_raw if isinstance(replicas_raw, int) else 0
