...
class RequestResponseTransformationMiddleware(Middleware):
    ...
    Currently a passthrough — reserved for future use (e.g., injecting
    namespace context, sanitizing sensitive fields).
    """
...
def setup_middleware(mcp: FastMCP, config: ServerConfig) -> None:
    ...
    # 5. Caching — K8s CRDs change less frequently than Prometheus metrics
    mcp.add_middleware(
        ResponseCachingMiddleware(
